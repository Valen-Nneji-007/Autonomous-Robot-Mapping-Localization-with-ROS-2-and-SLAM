...
#include "navbot_msgs/srv/add_two_ints.hpp"
...
public:
    ...
    {
        client_ = create_client<navbot_msgs::srv::AddTwoInts>("add_two_ints");

        auto request = std::make_shared<navbot_msgs::srv::AddTwoInts::Request>();
        request ->a =a;
        request ->b  = b;
...
private:
    rclcpp::Client<navbot_msgs::srv::AddTwoInts>::SharedPtr client_;

    void responseCallback(rclcpp::Client<navbot_msgs::srv::AddTwoInts>::SharedFuture future)
    {
        if(future.valid())
        {
