#include "navbot_localization/odometry_motion_model.hpp"
...

{
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("navbot_controller/odom", 10, std::bind(&OdometryMotionModel::odomCallback, this , _1));
...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry &odom)
{
    ...
    // the shows the noise free rot and it is gotten from substracting the noise from the noise estimation
    for(auto & sample : samples_.poses){
        ...
        // we need to take it back to Quaternion
        tf2::Quaternion q;
