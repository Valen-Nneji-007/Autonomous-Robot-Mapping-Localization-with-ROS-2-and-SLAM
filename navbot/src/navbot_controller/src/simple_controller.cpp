        ...
        , theta_(0.0)
{
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Using the wheel_radius " << wheel_radius_);
    RCLCPP_INFO_STREAM(get_logger(), "Using the wheel_separation " << wheel_separation_);

    wheel_cmd_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>("simple_velocity_controller/commands", 10);
    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("navbot_controller/cmd_vel", 10,
    ...
    odom_pub_ = create_publisher<nav_msgs::msg::Odometry><("/navbot_controller/odom", 10);
...
{
    if (first_joint_msg_) {
    prev_time_ = rclcpp::Time(msg.header.stamp);  // source 1
    ...
    if (dt.seconds() == 0.0) return;  // guard against duplicate stamps
